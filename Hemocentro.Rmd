...
sertao_santana <- doacoes %>%  
 subset(CIDADE == 'SERTAO SANTANA' & ABO_DOADOR != 'NULL')

head(sertao_santana)

# Primeiro convertemos a data de factor para Date
sertao_santana$DT_DOACAO <- as.Date(sertao_santana$DT_DOACAO, format = "%d/%m/%Y") 
head(sertao_santana)
sertao_santana$MONTH <- factor(month(sertao_santana$DT_DOACAO))
sertao_santana$YEAR <- factor(year(sertao_santana$DT_DOACAO))
...
df_idade %>%
  ggplot(mapping = aes(x = FAIXA, y = DOACOES/POPULACAO, fill=SEXO))+
    geom_bar(stat = "identity")+
    labs(title="Proporção de Doações por População Etária e Sexo - Sertão Santana")+
