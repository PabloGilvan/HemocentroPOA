...
Vamos agrupar os valors por tipo sanguíneo e verficar a proporção de cada um.

#### Proporção de Tipo sanguíneo
```{r}

tipo_sangue <- subset(doacoes, TIPO_SANGUE != 'NULL') %>%
  group_by(TIPO_SANGUE)%>%
  summarise(QUANTIDADE=n(),
            PROPORCAO = QUANTIDADE/nrow(subset(doacoes, TIPO_SANGUE != 'NULL')))
 
tipo_sangue%>%
 ggplot(mapping = aes(x = TIPO_SANGUE, y = PROPORCAO))+
 geom_bar(stat = "identity")+
 labs(title="Proporção de doação por tipo sanguíneo")+
 ylab("Quantidade")+
 xlab("Tipo Sanguíneo")+
...
#### Proporção de Tipo sanguíneo por Sexo
```{r}

tipo_sangue <- subset(doacoes, TIPO_SANGUE != 'NULL') %>%
  group_by(TIPO_SANGUE, SEXO)%>%
  summarise(QUANTIDADE=n(),
            PROPORCAO = QUANTIDADE/nrow(subset(doacoes, TIPO_SANGUE != 'NULL')))
 
tipo_sangue%>%
 ggplot(mapping = aes(x = TIPO_SANGUE, y = PROPORCAO, color=SEXO, fill=SEXO))+
 geom_bar(stat = "identity")+
 labs(title="Proporção de doação por tipo sanguíneo e sexo")+
 ylab("Quantidade")+
 xlab("Tipo Sanguíneo")+
 theme(plot.title = element_text(color="red", size=12, face="bold.italic"),
       axis.text.x = element_text(angle = 90, hjust = 1),
...
```{r}
tipo_sangue <- subset(doacoes, CIDADE == 'TUPANDI') %>%
  group_by(TIPO_SANGUE, SEXO)%>%
  summarise(QUANTIDADE=n())
 
tipo_sangue%>%
 ggplot(mapping = aes(x = TIPO_SANGUE, y = QUANTIDADE))+
 geom_bar(stat = "identity")+
...
```{r}
doacoes %>%  
 subset(CIDADE == 'TUPANDI') %>%
 group_by(TIPO, SEXO)%>%
 summarise(QUANTIDADE=n())%>%
 ggplot(mapping = aes(x = TIPO, y = QUANTIDADE))+
  geom_bar(stat = "identity")+
...
```{r}
doacoes_tupandi <- subset(doacoes, CIDADE == 'TUPANDI')
...
```{r}
doacoes %>%  
 subset(CIDADE == 'TUPANDI' & TIPO_SANGUE != 'NULL') %>%
 group_by(TIPO, TIPO_SANGUE)%>%
 summarise(QUANTIDADE=n())%>%
 ggplot(mapping = aes(x = TIPO, y = QUANTIDADE))+
  geom_bar(stat = "identity")+
  geom_text(aes(label=QUANTIDADE), vjust=0, color="red") +
